...
<div class="max-w-4xl mx-auto space-y-6 text-sm">
    <div class="flex items-center justify-between">
        <div>
            <h2 class="text-2xl font-bold text-gray-900 tracking-tight">Update Task</h2>
            <p class="text-sm text-gray-500 mt-1">Editing: {{ $task->title }}</p>
        </div>
        <a href="{{ route('tasks.show', [$company->slug, $task->id]) }}" class="inline-flex items-center gap-2 px-4 py-2 bg-white border border-gray-200 text-gray-700 text-sm font-bold rounded-xl hover:bg-gray-50 transition-all shadow-sm">
    ...
    <div class="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden text-sm">
        <form action="{{ route('tasks.update', [$company->slug, $task->id]) }}" method="POST" class="p-6 md:p-8 space-y-8">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <div class="md:col-span-2">
                    ...
                    <x-text-input id="title" name="title" type="text" class="mt-1 block w-full text-lg font-bold tracking-tight" :value="old('title', $task->title)" required />
                ...
                <div>
                    ...
                    <select id="parent_id" name="parent_id" class="mt-1 block w-full border-gray-200 focus:border-brand-500 focus:ring-brand-500 rounded-xl shadow-sm transition-all text-sm font-medium">
                        ...
                        @foreach($tasks as $pTask)
                            <option value="{{ $pTask->id }}" {{ old('parent_id', $task->parent_id) == $pTask->id ? 'selected' : '' }}>
                                {{ $pTask->title }}
                            </option>
                        @endforeach
                    </select>
            ...
            <div class="space-y-4">
                ...
                <textarea id="description" name="description" rows="5" class="mt-1 block w-full border-gray-200 focus:border-brand-500 focus:ring-brand-500 rounded-xl shadow-sm transition-all text-sm font-medium">{{ old('description', $task->description) }}</textarea>
            ...
            <div class="flex items-center justify-end gap-4 pt-6 border-t border-gray-50 uppercase tracking-widest font-bold">
                <a href="{{ route('tasks.show', [$company->slug, $task->id]) }}" class="text-xs text-gray-400 hover:text-gray-600 transition-colors">
                    Cancel
                </a>
                <x-primary-button>
                    Update Task
